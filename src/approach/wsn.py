...
#from networks.wsn_libs import set_model_task, set_num_tasks_learned, set_alphas, cache_masks, show_model_requires_grad, set_used_weights_fixed
...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5,
                 clipgrad=10000, momentum=0, wd=0, multi_softmax=False, wu_nepochs=0, wu_lr_factor=1,
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        #set_model_task(self.model.model, t)
        #
        ...
        for m in self.model.model.ListCustomModules():
            m.cache_masks()
        #set_num_tasks_learned(self.model.model, self.num_tasks_learned)
        #cache_masks(self.model.model)
...
#________________________________________________________________________
    ...
    def eval_taw(self, u, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                hits = self.calculate_metrics_wsn(outputs, targets)
                # Log
        ...
        for images, targets in val_loader:
            ...
            if inferred_task == u:
                ...
                with torch.no_grad():
                    ...
                    outputs = self.model(images.to(self.device))
                    hits = self.calculate_metrics_wsn(outputs, targets, taw_mode=False)
                    loss = self.criterion(u, outputs, targets.to(self.device))
            else:
    ...
    def calculate_metrics_wsn(self, outputs, targets, taw_mode=True):
    ...
    #
    # modification of train_epoch:
    # set_used_wreights_fixed() is inserted after loss.backward().
    # so that the gradients of the weights
    #   selected in the previous tasks shold be set to zero.
    #_______________________________________________________________
    #
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            #set_used_weights_fixed(self.model.model, verbose=False)
