...
class Appr(Inc_Learning_Appr):
    ...
    def post_train_process(self, t, trn_loader, val_loader):
        print('== Rank Reduction [task:%d] =='%(t))
        # フルランクでの loss値 と acc をベースラインとする
        best_loss, best_acc, _  = super().eval(t, val_loader)	# 本来は再計測しなくてよい。train_loopで算出済み⇒要修正

        ## Debug-0 (For Your Information)
        print('Debug-0:')
        print(F'  best_loss=%.3f,   best_acc=%.3f' %(best_loss, best_acc))
        print(F'loss_margin=%.3f, acc_margin=%.3f' % (self.loss_margin, self.acc_margin))
        for sow in self.model.model.ListCustomModules() :
            self.check_rank_of_SOW(sow)
        
        for sow in self.model.model.ListCustomModules() :
            #print(sow)
            #print('max_rank=%d' % (sow.N))
            # 減衰パラメータ(フルランク) sow.a のバックアップを保管
            sow.backup_full_rank_a()
            best_r = sow.max_rank
            for r in range(min(256, int(sow.max_rank/4)), sow.max_rank, 32):
                # 指定数 r まで低ランク化した減衰パラメータをバックアップから作り，sow.a に設定
                sow.set_low_rank_a(r)
                sow.rank = r
                loss, acc, _ = super().eval(t, val_loader)
                print(' r=%d, loss=%.3f, acc=%.3f (rank=%d)' % (r, loss, acc, sow.rank))
                #print(' loss_margin=%.3f, acc_margin=%.3f' %(self.loss_margin, self.acc_margin))
                if (acc >= best_acc - self.acc_margin) and (loss < best_loss + self.loss_margin) :
                    best_acc = acc
                    best_loss = loss
                    best_r = r
                    break
                if r > int(0.99 * sow.N) :
                    print('WARNING: Free Basis of SOW is not enough.')
                    continue
            print(' best_r=%d, loss=%.3f, acc=%.3f' % (r, loss, acc))
            sow.save_low_rank_a(t)	# 最善の低ランク a をタスク毎に保管
            sow.save_rank(t, r)		# タスク固有のランクを保存
            sow.update_freezed_rank(r)	# 必要なら固定ランクの更新
            #print('sow.a:', sow.a)
            #print(f'sow.low_rank_a[{t}]:')
            #print(sow.low_rank_a[t])
            #print(f'sow.ranks[{t}] =', sow.get_rank(t))
            #print(f' sow.rank=', sow.rank)
        
        print('== Header Training for Low Rank [task:%d] =='%(t))
        self.model.model.Fix_Classifier()

        ## Debug-1 (For Your Information)
        loss, acc, _ = super().eval(t, val_loader)
        print(F'Debug-1: loss=%.3f, acc=%.3f' %(loss, acc))
        for sow in self.model.model.ListCustomModules() :
            self.check_rank_of_SOW(sow)
            
        saved_lr = self.lr
        saved_sow_lr = self.sow_lr
        self.lr /= self.lr_factor
        self.sow_lr /= self.lr_factor
        self.train_loop(t, trn_loader, val_loader, loss)
        self.lr = saved_lr
        self.sow_lr = saved_sow_lr

        ## Debug-2 (For Your Information)
        loss, acc, _ = super().eval(t, val_loader)
        print(F'Debug-2: loss=%.3f, acc=%.3f' %(loss, acc))
        for sow in self.model.model.ListCustomModules() :
            self.check_rank_of_SOW(sow)
        ...
        for sow in self.model.model.ListCustomModules() :
            sow.restore_full_rank_a()	# 退避していた減衰パラメータ（フルランク）を設定
            sow.rank = sow.max_rank	# 次タスクでの学習や推論に備えてランクをフルに回復
        self.model.model.Free_Classifier() # Never set 'requires_grad=True' for low_rank_a[t]

        ## Debug-3 (For Your Information)
        loss, acc, _ = super().eval(t, val_loader)
        print(F'Debug-3: loss=%.3f acc=%.3f' %(loss, acc))
        for sow in self.model.model.ListCustomModules() :
            self.check_rank_of_SOW(sow)

        print('Header Training: Finished.')
        return
