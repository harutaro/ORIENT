...
def get_data(trn_data, tst_data, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    ...
    """タスクごとのデータセットのデータ、タスクごとのクラス数、クラスのオーダを返す

    1. 引数で与えられたクラス数で連番のリストを作成し、シャッフル
    2. タスクごとのクラス数の累積和のリストを作成
    3. 1つのデータのペア(訓練、正解)ごとに、1.で作成したリストの要素と一致するインデックスを取得
    4. 3.のインデックスから2.を使用して格納するタスクを決定し、タスクごとにデータを割り振る
    5. 4.を各タスク毎に引数で与えられた検証用データのパーセンテージ(デフォルト: 0.1)によって、訓練データ(0.9)と検証データ(0.1)で重複無しでランダムに分ける



    Args:
        trn_data (VisionDataset): trainデータセット
        tst_data (VisionDataset): testデータセット
        num_tasks (int): タスク数
        nc_first_task (int): 第1タスクのクラス数
        validation (float): 検証データの割合
        shuffle_classes (bool): クラスをシャッフルするかのフラグ
        class_order (list[int], optional): クラスのオーダ. Defaults to None.

    Returns:
        tuple[dict[Union[int, str], Any], list[tuple[int]], list[int]]: タスクごとのデータセットのデータ、タスクごとのクラス数、クラスのオーダ
    """
