...
class ResNet50_32SOW(nn.Module):
    """ 入力：3ch × 32 画素 × 32 画素 """
    """ アーキテクチャ： ＳＯＷ版ＲｅｓＮｅｔ５０ """
    ...
    def __init__(self, block: Union[BasicBlock, BottleNeck], num_block: list,
                 ...
                 load_features=False, pretrained_path='', num_tasks=1, **kwargs):
        super().__init__()

        N = int(512*block.expansion)
        M = int(math.log(N, 2))
        '''
        ...
        tan_2phi = sow_dict['tan_2phi'].to(device=device)
        tan_2theta = sow_dict['tan_2theta'].to(device=device)
        ...
        self.classifier = nn.Sequential(
            ...
            SOW_V3(M, '../src/networks/INIT/1024.dict', num_tasks, device=device, dtype=torch.float64),
        ...
        if pretrained_path:
            ...
            if load_features:
                ...
                print(F'ResNet50_32SOW,__init__: features is loaded ({pretrained_path})')
    ...
    def ListCustomModules(self):
        for m in self.classifier.modules():
            if isinstance(m, SOW_V3) : yield m
        return

    def IndicesSOW(self):
        for i, m in enumerate(self.classifier.modules()):
            if isinstance(m, SOW_V3) : yield i
        return
...
def resnet50_32sow(device=None, **kwargs):
    return ResNet50_32SOW(BottleNeck, [3, 4, 6, 3], device, **kwargs)
