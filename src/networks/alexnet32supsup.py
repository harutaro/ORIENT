...
## for SUPSUP class
from .supsup_libs import SupSup_MaskLinear

__all__ = ("alexnet_32supsup")

class AlexNet_32SUPSUP(nn.Module):
    ''' input size: 3 channels x 32 pixels x 32 pixels '''
    ''' replaced nn.Linears with SupSup_MaskLinear in the classifier block. '''
    ...
    def __init__(self, device='cuda', num_classes=1000, dropout=0.5,
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(p=d1),
            SupSup_MaskLinear(256*4*4, 4096, bias=False, num_tasks=num_tasks, device=device, dtype=torch.float64),
            nn.ReLU(),
            nn.Dropout(p=d2),
            SupSup_MaskLinear(4096, 4096, bias=False, num_tasks=num_tasks, device=device, dtype=torch.float64),
            nn.ReLU()
        )
    ...
    def ListCustomModules(self):
        for m in self.classifier.modules():
            if isinstance(m, SupSup_MaskLinear) : yield m

    def IndicesSupSup(self):
        for i, m in enumerate(self.classifier.modules()):
            if isinstance(m, SupSup_MaskLinear) : yield i
...
def alexnet_32supsup(device=None, **kwargs):
    return AlexNet_32SUPSUP(device, **kwargs)
