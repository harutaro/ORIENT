# Initial Weight Matrix W（= Uh・Σ・V）の U から tan 2φ を求める
# ex) python save_U.py 0 10 512 20.0 0.90 1.0
...
class Givens_U_Function(Function):
    """Output & Gradient Calcuation for Y=X*U^T """
    @staticmethod
    def forward(ctx, X, tan_2phi, ptbl):
        ...
        cos_phi = torch.empty((1, int(N*(N-1)/2)), dtype=X.dtype, device=X.device, requires_grad=False)
        sin_phi = torch.empty((1, int(N*(N-1)/2)), dtype=X.dtype, device=X.device, requires_grad=False)
        ...
        sow_cpp.forward_Uh(X, ptbl, rank_T, tan_2phi, cos_phi, sin_phi, ZZ, Y)
    ...
    def backward(ctx, grad_Y):
        ...
        sow_cpp.backward_Uh(grad_Y, ptbl, rank_T, freezed_rank_T, tan_2phi,
                            cos_phi, sin_phi, ZZ, grad_X, grad_tan_2phi)
        ...
        # ptbl has no grad.
        return grad_X, grad_tan_2phi, None

class Givens_U_Net(nn.Module):
    def __init__(self, M, ptbl, device=None, dtype=None):
        ...
        self.U_func = Givens_U_Function.apply
    ...
    def reset_parameters(self, M, ptbl):
        ...
        self.N = N
        # tan_2phi の初期値を設定
        #   本来 phi は -pi/4 [rad] 以上, +pi/4 未満
        rotation_params_num = int(N*(N-1)/2)
        # パラメータ tan_2phi の設定
        phi = torch.zeros(rotation_params_num, dtype=dtype, device=device)
        self.tan_2phi = nn.Parameter(torch.tan(2.0 * phi))
        # ptbll の設定（それぞれはクラス変数．SOWのインスタンスすべてで共通）
        assert ptbl.shape[0] == int(N*(N-1))
        self.ptbl = ptbl

    def forward_Givens_U(self, X):
        # Givens Rotation Matrixes の収束確認を行うために使用
        Y = self.U_func(X, self.tan_2phi, self.ptbl)
        return Y
...
def main(device, M, BS, lr, momentum, clip):
    ...
    # モデル構築 (Uh)
    model = Givens_U_Net(M, ptbl, device, dtype)
    ...
    for step in range(max_step):
        ...
        t2 = time.time()
        optimizer.zero_grad()
        Y_est = model.forward_Givens_U(X)
        loss = torch.sum((Y_est - Y)*(Y_est - Y)) / (N * BS)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.tan_2phi, clip)
        optimizer.step()
        if loss < best_loss :
            best_loss = loss
            best_model = model.get_copy()
            print(f'[U] %8d %4d %4d lr=%.3e loss=%.4e time=%.3f *'
        ...
        print('X:', X.shape)
        print(X)
        print('tan_2phi:', model.tan_2phi.shape)
        print(model.tan_2phi)
        print('-------')
        exit()
        '''
    #
    model.set_state_dict(best_model)
    sow_dict = dict()
    sow_dict['tan_2phi'] = model.tan_2phi.to('cpu')
    #
    ...
    #
    torch.save(sow_dict, f'networks/INIT/U%04d.dict' % (N))
